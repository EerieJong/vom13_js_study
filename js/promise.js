...
let d = true;
const getData = new Promise((resolve, reject) => {  // new Promise 는 1급 객체 인데 1급 객체란 변수에 대입할 수 있는 "값"을 뜻한다.
  if(d){
    resolve('데이터 수신 성공 :o');
  } else {
    reject('데이터 수신 실패 :p');
...
// 2. 정의된 내용을 실행한다. 이때 사용하는 키워드는 then과 catch다.  // 체계적이면서 반복적으로 쓸 수 도 있음. 
// getData
//   .then((result) => console.log(result))
//   .catch((error) => console.log(error))
//   .finally(() => console.log('완료 :)'));

  // console.log(getData); // reject : rejected, resolve : fulfilled /// 정의되지 않은 promise : pending(준비단계)

//3. 콜백 함수가 여러개여서 함수 중첩이 많아질 경우(콜백지옥) 프로미스 체이닝을 사용한다.
// function step1(cb){} 화살표 함수 풀어놓은거  
...
const step1 = (msg) => {
  console.log(msg);
  return new Promise((resolve, reject) => {
    setTimeout(() =>{resolve('print step1')}, 1000);
  });
}
...
const step2 = (msg) => {
  console.log(msg);
  return new Promise((resolve, reject) => {
    setTimeout(() =>{resolve('print step2')}, 1000);
  });
}
...
const step3 = (msg) => {
  console.log(msg);
  return new Promise((resolve, reject) => {
    setTimeout(() =>{resolve('print step3')}, 1000);
  });
}
