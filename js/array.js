...
// 요소로 등록되는 데이터는 모든 타입이 가능하나 대부분 "원시" 리터럴(숫자, 문자) 타입을 사용한다.  // 함수 형태는 일관성이 없어서 비추
...
// 5.1 배열 요소의 추가 : push
// push() 메서드는 배열의 "뒷부분"에 요소를 추가한다.
// 이 경우 원본 데이터가 변경된다. //깊은복사 얕은복사 참조
...
// 문자열을 읽고, 특정 문자(parameter)를 기준으로 투 개의 배열을 생성한다.

const url = 'http://www.abc.com?q=3'
const qarr = url.split('=')
// console.log(Number(qarr[1]));
